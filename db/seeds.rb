...
def vote_already_saved?(array, bill_id, voter_id)
  array.each do |subarray|
    return true if subarray.include?(bill_id) && subarray.include?(voter_id)
  end
  false
end 
...
vote_data.each.with_index do |page, i|
  page["results"].each do |vote|
    ...
    vote["voter_ids"].each do |voter, type|
      ...
      v.vote_type = type
      v.voted_at = voted_at
      v.save unless vote_already_saved?(been_dun_part_revenge, bill_id, voter)
      been_dun_part_revenge << [bill_id, voter]
    end
  end
